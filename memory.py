"""
AKARI Memory & Planning — persistent context, tasks, notes, and smart routing.

Three systems:
1. Memory — facts, preferences, project context AKARI learns from conversations
...
so AKARI gets smarter over time.
"""
...
log = logging.getLogger("akari.memory")

DB_PATH = Path(__file__).parent / "data" / "akari.db"
...
def format_plan_for_voice(tasks: list[dict], events: list[dict]) -> str:
    ...
    if not tasks and not events:
        return "Your day looks clear! No events or tasks scheduled."
...
async def extract_memories(user_text: str, akari_response: str, anthropic_client) -> list[str]:
