...
# ---------------------------------------------------------------------------
# Config Ensuring (Safe for package or direct run)
# ---------------------------------------------------------------------------
try:
...
# Import Akari modules after config is ensured
sys.path.append(os.path.dirname(os.path.abspath(__file__)))

try:
    from .planner import TaskPlanner, detect_planning_mode, PlanningDecision
    from .server import extract_action, AKARI_SYSTEM_PROMPT, ClaudeTaskManager
    from .memory import remember, recall, create_task, get_open_tasks, search_notes
    from .config import ensure_config
except ImportError:
    # If not running as a package, try absolute
    try:
        from planner import TaskPlanner, detect_planning_mode, PlanningDecision
        from server import extract_action, AKARI_SYSTEM_PROMPT, ClaudeTaskManager
        from memory import remember, recall, create_task, get_open_tasks, search_notes
...
# Ensure config at the very beginning
try:
    from .config import ensure_config
    ensure_config()
except ImportError:
...
# ---------------------------------------------------------------------------
# Config for Voice (Read again after ensure_config)
# ---------------------------------------------------------------------------
...
class AkariCLI:
    def __init__(self):
        ...
        if not self.api_key:
            # This shouldn't happen if ensure_config worked, but just in case
            console.print("[red]ANTHROPIC_API_KEY not found. Please set it in your .env file.[/red]")
            sys.exit(1)
...
\____|__  /|__|_ \  |______/|______/|__|
    ...
    async def handle_command(self, cmd: str):
        ...
        if cmd == "/help":
            ...
            table.add_row("/auth", "Update API keys and username")
            ...
            console.print(Panel(Text("Authentication & Settings 🌸", style="magenta"), border_style="magenta"))
            
            choice = Prompt.ask(
                "What would you like to update?",
                choices=["anthropic", "fish", "username", "cancel"],
                default="cancel"
            )
            
            if choice == "anthropic":
                new_key = Prompt.ask("[bold cyan]Enter new Anthropic API Key[/bold cyan]", password=True)
                if new_key:
                    save_config_value("ANTHROPIC_API_KEY", new_key)
                    self.api_key = new_key
                    self.client = anthropic.AsyncAnthropic(api_key=new_key)
                    console.print("[green]Anthropic API Key updated! ✨[/green]")
            elif choice == "fish":
                new_key = Prompt.ask("[bold cyan]Enter new Fish Audio API Key[/bold cyan]", password=True)
                if new_key:
                    save_config_value("FISH_API_KEY", new_key)
                    global FISH_API_KEY
                    FISH_API_KEY = new_key
                    console.print("[green]Fish Audio API Key updated! ✨[/green]")
            elif choice == "username":
                new_name = Prompt.ask("[bold cyan]What should I call you?[/bold cyan]")
                if new_name:
                    save_config_value("USER_NAME", new_name)
                    self.user_name = new_name
                    console.print(f"[green]Username updated to {new_name}! ✨[/green]")
    ...
    async def main_loop(self):
        ...
        if welcome_file_path.exists():
            try:
                ...
                console.print(f"[dim]Debug: Audio play error: {e}[/dim]")
        else:
            console.print(f"[dim]Debug: Audio not found at {welcome_file_path}[/dim]")
